                    ...
                    if (event.startDate - now < 2 * oneDay) {
                        ...
                        if (this.config.timeFormat === "absolute") {
                            if ((this.config.urgency > 1) && (event.startDate - now < (this.config.urgency * oneDay))) {
                                ...
                                var startDate = moment(event.startDate, "x").format(this.config.dateFormat);
                                var endDate   = moment(event.endDate, "x").format(this.config.dateFormat);

                                timeWrapper.innerHTML = this.capFirst(startDate);

                                if (startDate !== endDate) {
                                    timeWrapper.innerHTML += ' au ' + this.capFirst(endDate);
                                }
                            }
                        } else {
                            timeWrapper.innerHTML = this.capFirst(moment(event.startDate, "x").fromNow());
